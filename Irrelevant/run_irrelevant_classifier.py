...
def predict(input_file, model_path, output_file, text_column="Description"):
    ...
    if not os.path.exists(model_path):
        ...
        # We proceed only if the user passed a huggingface hub model name,
        # otherwise this will crash in from_pretrained.
        pass
    ...
    for i in tqdm(range(0, len(texts), batch_size)):
        ...
        with torch.no_grad():
            ...
            probabilities_class_1.extend(probs[:, 1].cpu().tolist())

    df["predicted_label"] = predictions
    df["probability_irrelevant"] = (
        probabilities_class_1  # Assuming class 1 is irrelevant as per train script
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_file", default="results.csv", help="Path to save results"
    )
    parser.add_argument(
        "--text_column",
        default="Description",
